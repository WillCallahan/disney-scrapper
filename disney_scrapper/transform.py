...
def _transform_resorts_and_reservations(resorts, reservations, max_price, exclude_resorts):
    ...
    joined_resorts_filtered = list(filter(lambda r: not any([e in r.get('resort', {}).get('name') for e in exclude_resorts]), joined_resorts))
...
def _frequency_report(all_availabilities):
    resort_types = [r.get('resort', {}).get('facets', {}).get('resortCategory', {})[0].get('value') for r in all_availabilities]
...
def transform_to_message(resorts, reservations, max_price, exclude_resorts, start_date, end_date):
